...
pub trait InputFor<'src, ImplicitBounds: Sealed = Bounds<&'src Self>> {
    ...
    ///
    /// ```ignore
    /// type MaybeToken = TokenOf<'src,Self>;
    /// ```
    ///
    /// or
    ///
    /// ```ignore
    /// type MaybeToken = &'src TokenOf<'src,Self>;
    /// ```
    ///
...
pub type SpanOf<'src, I> = <I as InputFor<'src>>::Span;
pub type CacheOf<'src, I> = <I as InputFor<'src>>::Cache;
pub type CursorOf<'src, I> = <I as InputFor<'src>>::Cursor;
pub type MaybeTokenOf<'src, I> = <I as InputFor<'src>>::MaybeToken;
pub type TokenOf<'src, I> = <I as InputFor<'src>>::Token;
...
pub trait Input: for<'src> InputFor<'src> {
    ...
    where
        Self: Input + Sized,
        F: for<'src> SpanMapper<'src, SpanOf<'src, Self>, S>,
    {
        MappedSpan {
            input: self,
            map_fn,
...
pub trait SliceInputFor<'src, ImplicitBounds: Sealed = Bounds<&'src Self>> {
...
pub trait SliceInput: ExactSizeInput + for<'any> SliceInputFor<'any> {
...
impl<'src, T, const N: usize> InputFor<'src> for &'_ [T; N] {
    type Cursor = usize;
    type Span = SimpleSpan<usize>;
    ...
    M: for<'any> InputMapper<'any,'map, MaybeTokenOf<'any, I>, T, S>,
{
    ...
    type MaybeToken = <MaybeTokenOf<'src, I> as IntoMaybe<'src, TokenOf<'src, I>>>::Proj<T>;
...
impl<'src, I, S, F> InputFor<'src> for MappedSpan<S, I, F>
where
    I: Input,
    F: for<'any> SpanMapper<'any, SpanOf<'any, I>, S>,
    S: Span,
{
    type Span = S;

    type Token = TokenOf<'src, I>;
...
where
    S: Span + Clone,
    S::Context: Clone,
    S::Offset: for<'src> From<<SpanOf<'src, I> as Span>::Offset>,
    F: for<'any> SpanMapper<'any, SpanOf<'any, I>, S>,
{
    #[inline(always)]
    fn begin<'src>(self) -> (CursorOf<'src, Self>, CacheOf<'src, Self>) {
...
where
    ...
    S: Span + Clone,
    S::Context: Clone,
    for<'any> S::Offset: From<<SpanOf<'any, I> as Span>::Offset>,
    F: for<'any> SpanMapper<'any, SpanOf<'any, I>, S>,
{
    #[inline(always)]
    unsafe fn span_from<'src>(
...
impl<'src, S, I, F> SliceInputFor<'src> for MappedSpan<S, I, F>
where
    I: SliceInput,
    S: Span,
    F: for<'any> SpanMapper<'any, SpanOf<'any, I>, S>,
...
where
    ...
    S: Span + Clone,
    S::Context: Clone,
    for<'src> TokenOf<'src, I>: Char,
    for<'src> S::Offset: From<<SpanOf<'src, I> as Span>::Offset>,
    F: for<'any> SpanMapper<'any, SpanOf<'any, I>, S>,
{
    #[doc(hidden)]
    fn stringify<'src>(slice: SliceOf<'src, Self>) -> String {
...
impl<'src, I: Input, S: Span> InputFor<'src> for WithContext<S, I> {
    type Cursor = CursorOf<'src, I>;
    type Span = S;
...
impl<'src, S, I> SliceInputFor<'src> for WithContext<S, I>
where
    I: SliceInput,
    S: Span,
{
...
impl<R: Read + Seek> InputFor<'_> for IoInput<R> {
    type Cursor = usize;
...
/// `parse`.
pub(crate) struct InputOwn<'src, 's, I: Input, E: ParserExtra<'src, I>> {
    pub(crate) start: CursorOf<'src, I>,
    pub(crate) cache: CacheOf<'src, I>,
    pub(crate) errors: Errors<CursorOf<'src, I>, E::Error>,
    pub(crate) state: MaybeMut<'s, E::State>,
    pub(crate) ctx: E::Context,
    #[cfg(feature = "memoization")]
    pub(crate) memos: HashMap<(usize, usize), Option<Located<CursorOf<'src, I>, E::Error>>>,
}
...
where
    I: Input,
    E: ParserExtra<'src, I>,
    TokenOf<'src, I>: IntoMaybe<'src, TokenOf<'src, I>>,
{
...
pub struct InputRef<'src, 'parse, I: Input, E: ParserExtra<'src, I>> {
    cursor: CursorOf<'src, I>,
    pub(crate) cache: &'parse mut CacheOf<'src, I>,
    pub(crate) errors: &'parse mut Errors<CursorOf<'src, I>, E::Error>,
    ...
    TokenOf<'src, I>: IntoMaybe<'src, TokenOf<'src, I>>,
{
    ...
    #[inline]
    pub(crate) fn skip_while<F: FnMut(&TokenOf<'src, I>) -> bool>(&mut self, mut f: F)
    where
        I: Input,
    {
        loop {
    ...
    #[inline(always)]
    pub(crate) fn next_inner(&mut self) -> Option<TokenOf<'src, I>>
    where
        I: ValueInput,
    {
        // SAFETY: cursor was generated by previous call to `Input::next`
    ...
    #[inline(always)]
    pub(crate) fn next_ref_inner(&mut self) -> Option<&'src TokenOf<'src, I>>
    where
        I: BorrowInput,
    {
        // SAFETY: cursor was generated by previous call to `Input::next`
    ...
    #[inline(always)]
    pub fn next_maybe(&mut self) -> Option<MaybeRef<'src, TokenOf<'src, I>>> {
    ...
    /// See [`InputRef::next_ref`] if you want get a reference to the next token instead.
    #[inline(always)]
    pub fn next(&mut self) -> Option<TokenOf<'src, I>>
    where
        I: ValueInput,
    {
        self.next_inner()
    ...
    #[inline(always)]
    pub fn next_ref(&mut self) -> Option<&'src TokenOf<'src, I>>
    where
        I: BorrowInput,
    {
        self.next_ref_inner()
    ...
    #[inline(always)]
    pub fn peek_maybe(&mut self) -> Option<MaybeRef<'src, TokenOf<'src, I>>> {
        // SAFETY: cursor was generated by previous call to `Input::next`
    ...
    /// Peek the next token in the input. Returns `None` if the end of the input has been reached.
    #[inline(always)]
    pub fn peek(&mut self) -> Option<TokenOf<'src, I>>
    where
        I: ValueInput,
    {
        // SAFETY: cursor was generated by previous call to `Input::next`
    ...
    /// Peek the next token in the input. Returns `None` if the end of the input has been reached.
    #[inline(always)]
    pub fn peek_ref(&mut self) -> Option<&'src TokenOf<'src, I>>
    where
        I: BorrowInput,
    {
        // SAFETY: cursor was generated by previous call to `Input::next`
    ...
    #[inline(always)]
    pub fn span_from(&mut self, range: RangeFrom<&Cursor<'src, 'parse, I>>) -> SpanOf<'src, I>
    where
        I: ExactSizeInput,
    {
        // SAFETY: `Cursor` is invariant over 'parse, so we know that this cursor came from the same input
    ...
    #[inline(always)]
    pub fn span_since(&mut self, before: &Cursor<'src, 'parse, I>) -> SpanOf<'src, I> {
        // SAFETY: `Cursor` is invariant over 'parse, so we know that this cursor came from the same input
        // See `https://plv.mpi-sws.org/rustbelt/ghostcell/`
    ...
    pub(crate) fn fail_with<D: Driver, O, Exp, L>(
        ...
        mk: impl FnOnce(
            ...
            Exp,
            Option<MaybeRef<'src, TokenOf<'src, I>>>,
            SpanOf<'src, I>,
        ),
    ) -> PResult<D::Mode, O>
    where
    ...
    pub(crate) fn fail_rewind_with<D: Driver, Exp, L, O>(
        ...
        mk: impl FnOnce(&mut Self, &SpanOf<'src, I>) -> (Exp, Option<MaybeRef<'src, TokenOf<'src, I>>>),
    ) -> PResult<D::Mode, O>
    where
        Exp: IntoIterator<Item = L>,
        E::Error: LabelError<'src, I, L>,
    ...
    #[inline]
    pub(crate) fn add_alt_err_impl<D: Driver>(&mut self, at: &CursorOf<'src, I>, err: E::Error) {
        if D::Policy::STRICT {
            return;
        }
        // Fast path: if the error doesn't carry meaningful information, avoid unnecessary decision-making!
    ...
    pub(crate) fn take_alt(&mut self) -> Option<Located<CursorOf<'src, I>, E::Error>> {
...
/// See [`Parser::map_with`].
pub struct MapExtra<'src, 'b, I: Input, E: ParserExtra<'src, I>> {
    before: &'b CursorOf<'src, I>,
    after: &'b CursorOf<'src, I>,
    cache: &'b mut CacheOf<'src, I>,
    state: &'b mut E::State,
    ctx: &'b E::Context,
    emitted: &'b mut Errors<CursorOf<'src, I>, E::Error>,
}

impl<'src, 'b, I: Input, E: ParserExtra<'src, I>> MapExtra<'src, 'b, I, E> {
    #[inline(always)]
    ...
    #[inline(always)]
    pub fn span(&mut self) -> SpanOf<'src, I> {
