    ...
    E: ParserExtra<I>,
{
    ...
    fn go<'src, D: Driver>(&self, inp: &mut InputRef<'src, '_, I, E>) -> PResult<D::Mode, ()> {
        ...
        match inp.next_maybe_inner() {
            None => Ok(D::Mode::bind(|| ())),
            Some(tok) => inp.fail_rewind_with::<D, _, (), _>(before, |_inp, _span| {
...
{
    #[inline]
    fn go<'src, D: Driver>(&self, _: &mut InputRef<'src, '_, I, E>) -> PResult<D::Mode, ()> {
        Ok(D::Mode::bind(|| ()))
    }
    ...
    T: for<'src> OrderedSeq<'src, I::Token> + Clone,
{
    ...
    #[inline]
    fn go<'src, D: Driver>(&self, inp: &mut InputRef<'src, '_, I, E>) -> PResult<D::Mode, T> {
    ...
    fn go_cfg<'src, D: Driver>(
        ...
        for next in seq.seq_iter() {
            ...
            match inp.next_maybe_inner() {
                ...
                found => {
                    return inp.fail_rewind_with::<D, _, Id<T>, _>(before, |_inp, _span| {
                        (
...
impl<I, E, T> Parser<I, Id<I::Token>, E> for OneOf<T, I, E>
where
    I: ValueInput,
    E: ParserExtra<I>,
    I::Token: PartialEq,
    T: for<'src> Seq<'src, I::Token>,
{
    #[doc(hidden)]
    #[cfg(feature = "debug")]
    fn node_info(&self, scope: &mut debug::NodeScope) -> debug::NodeInfo {
    ...
    fn go<'src, D: Driver>(
        &self,
        inp: &mut InputRef<'src, '_, I, E>,
    ) -> PResult<D::Mode, I::Token> {
        let before = inp.save();
        match inp.next_inner() {
            Some(tok) if self.seq.contains(tok.borrow()) => Ok(D::Mode::bind(|| tok)),
            found => inp.fail_rewind_with::<D, _, Id<I::Token>, _>(before, |_inp, _span| {
...
impl<I, E, T> Parser<I, Id<I::Token>, E> for NoneOf<T, I, E>
where
    I: ValueInput,
    E: ParserExtra<I>,
    I::Token: PartialEq,
    ...
    fn go<'src, D: Driver>(
        ...
        match next {
            Some(tok) => {
                ...
                inp.fail_rewind_with::<D, _, O, _>(before, |_inp, _span| {
                    ([DefaultExpected::SomethingElse], Some(tok.into()))
                })
            }

            None => inp.fail_rewind_with::<D, _, O, _>(before, |_inp, _span| {
...
/// See [`select_ref!`].
pub const fn select_ref<F, I, O, E>(filter: F) -> SelectRef<F, I, O, E>
where
    I: BorrowInput,
    E: ParserExtra<I>,
    F: for<'src> Fn(&I::Token, &mut MapExtra<'src, '_, I, E>) -> Option<O>,
...
impl<I, O, E, F> Parser<I, O, E> for SelectRef<F, I, O, E>
where
    I: BorrowInput,
    E: ParserExtra<I>,
    F: for<'src> Fn(&I::Token, &mut MapExtra<'src, '_, I, E>) -> Option<O::Of<'src>>,
    ...
    E: ParserExtra<I>,
{
    ...
    fn go<'src, D: Driver>(
        ...
        match inp.next_inner() {
            ...
            found => inp.fail_rewind_with::<D, _, Id<_>, _>(before, |_inp, _span| {
                let found = found.map(|f| f.into());
                ([DefaultExpected::Any], found)
            }),
        }
...
impl<I, E> Parser<I, RefFam<I::Token>, E> for AnyRef<I, E>
where
    I: BorrowInput,
    E: ParserExtra<I>,
{
    ...
    fn go<'src, D: Driver>(
        &self,
        inp: &mut InputRef<'src, '_, I, E>,
    ) -> PResult<D::Mode, Ref<'src, I::Token>> {
        let before = inp.save();
        match inp.next_ref_inner() {
            Some(tok) => Ok(D::Mode::bind(|| Ref::from(tok))),
            found => inp.fail_rewind_with::<D, _, RefFam<I::Token>, _>(before, |_inp, _span| {
                let found = found.map(|f: &_| f.into());
                ([DefaultExpected::Any], found)
            }),
        }
    }

    go_extra!(RefFam<I::Token>);
}
...
/// ```
pub const fn any_ref<'src, I: BorrowInput, E: ParserExtra<I>>() -> AnyRef<I, E> {
    AnyRef {
        phantom: EmptyPhantom::new(),
    }
}
...
where
    ...
    A: Parser<I, O, EI>,
    F: for<'src> Fn(&CtxOf<'src, I, E>, Lt<'src>) -> CtxOf<'src, I, EI>,
    O: Hkt,
{
    #[inline]
    fn go<'src, D: Driver>(
        ...
        inp.with_ctx(&(self.mapper)(inp.ctx(), Lt::new()), |inp| {
            self.parser.go::<D>(inp)
        })
    }
...
fn go_or_finish<'src, O, I, E, P, D>(
    item: &mut Option<<D::Mode as Mode>::Output<O::Of<'src>>>,
    parser: &P,
    inp: &mut InputRef<'src, '_, I, E>,
) -> PResult<D::Mode, ()>
where
...
fn go_or_rewind<'src, O: Hkt, I, E, P, D>(
    item: &mut Option<<D::Mode as Mode>::Output<O::Of<'src>>>,
    parser: &P,
    inp: &mut InputRef<'src, '_, I, E>,
) where
...
//         {
//             #[inline]
//             fn go<'src,D: Driver>(&self, inp: &mut InputRef<'src, '_, I, E>) -> PResult<D::Mode, ($($O,)*)> {
    ...
    E: ParserExtra<I>,
{
    ...
    fn go<'src, D: Driver>(
        &self,
        inp: &mut InputRef<'src, '_, I, E>,
    ) -> PResult<D::Mode, Vec<O::Of<'src>>> {
        let mut tmp = self
            .parsers
            .iter()
            .map(|_| None)
            .collect::<Vec<Option<<D::Mode as Mode>::Output<O::Of<'src>>>>>();

        // first iterate until there are no progress
        loop {
            let start = inp.cursor();
            for (i, parser) in self.parsers.iter().enumerate() {
                go_or_rewind::<O, _, _, _, D>(&mut tmp[i], parser, inp);
            }
            // none matched during this loop
            if start == inp.cursor() {
                break;
        ...
        // Then a final iteration that matches remaining empty parsers
        for (i, parser) in self.parsers.iter().enumerate() {
            go_or_finish::<O, _, _, _, D>(&mut tmp[i], parser, inp)?;
        }
    ...
    O: Hkt,
{
    ...
    fn go<'src, D: Driver>(
        ...
        let mut tmp: [Option<<D::Mode as Mode>::Output<O::Of<'src>>>; N] = [const { None }; N];
        ...
        // unwrap is ok since we matched all items in the se
        Ok(D::Mode::array(tmp.map(|x| x.unwrap())))
    }
